...
export class ComprobanteService {
    ...
    try {
      ...
      const where: any = {
        ...
        ...(['INFORMAL', 'TODOS'].includes(tipoComprobante) && estadoPago
          ...
          : {}),
        // Flujo comercial de Kaiser: filtro por estado del pedido
        ...(estadoPedido && estadoPedido !== 'TODOS'
          ? { estadoPedido: estadoPedido as any }
          : {}),
  ...
  private async buildPdfBufferInformal(
    ...
    if (full.tipoDoc === 'COT') {
      ...
      // ── Formato Kaiser: helpers y estructuras derivadas ─────────────────
      // Montos con separador de miles (p. ej. 12,568.24 / 53.780).
      const fmt2 = (n: any) =>
      ...
      const mapCuenta = (c: any) => ({
        banco: (c.banco || '').toUpperCase(),
        numeroCuenta: c.numeroCuenta || '',
        cci: c.cci || '',
      });
      const cuentasDolares = cuentasCfg
        .filter((c) => c.moneda === 'USD')
        .map(mapCuenta);
      const cuentasSoles = cuentasCfg
        .filter((c) => c.moneda !== 'USD')
        .map(mapCuenta);
      ...
      // Bloques configurables por empresa. Los textos editables (autorizado por,
      ...
      const cotizacionData = {
        ...
        cuentasDolares,
        cuentasSoles,
        monedaTexto: cotizEsUSD ? 'DOLARES AMERICANOS' : 'SOLES',
        monedaCodigo: cotizEsUSD ? 'USD' : 'PEN',
        valorVentaTotal: fmt2(full.mtoOperGravadas),
        adelantos: fmt2(adelantoMonto),
        ...
        contactoDireccion: (
          (full.cliente as any)?.contactoDireccion || ''
        ).toUpperCase(),
        horarioAtencion:
          textos.horario ||
          emp.horarioAtencion ||
        ...
        avisoObservaciones:
          textos.avisoObservaciones ||
          'LAS OBSERVACIONES DEL PRODUCTO SE RECIBIRÁN HASTA 7 DÍAS CALENDARIO; LUEGO SE CONSIDERARÁ CONFORME.',
        condicionNota:
          textos.condicionNota ||
          'PRECIO SUJETO A VARIACIÓN, POR LAS CONSTANTES ALZAS DE LA MATERIA PRIMA A NIVEL MUNDIAL. ANTES DE REALIZAR EL DEPÓSITO, CONSULTE TIPO DE CAMBIO DEL DÍA.',
        autorizadoNombre: textos.autorizadoNombre || 'CECILIA KAISER POLO',
        autorizadoCargo: textos.autorizadoCargo || 'Gerente Comercial',
        autorizadoTelefono: textos.autorizadoTelefono || '989007725',
        autorizadoEmail: textos.autorizadoEmail || 'ckp@kaisercorp.com.pe',
        emitidoNombre: (usuarioNombre || 'ADMIN').toUpperCase(),
        ...
        descuento: full.mtoDescuentoGlobal
          ...
          : undefined,
        validez: `${full.cotizVigencia || 7} DÍAS ÚTILES`,
